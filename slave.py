...
def write_shufffiles(hash_files, hostname, hosts_list, i):
    ...
    shuffle_file = (
        shuffle_path + hosts_list[i] + "/" + hostname + "-" + str(rand_num) + ".txt"
    )
    text = ""
...
def prep_shuffle(hostname, filename, shuffle_path, hosts_list):
    ...
    with open(filename, errors="ignore") as f:
        for line in f:
            word = line.split()[0]
            if word in hash_files.keys():
                hash_files[word] += 1
            else:
                hash_files[word] = 1
    if DEBUG:
        end = time.time()
        print(f"Done counting! {end-start}s")
        start = time.time()

    n = len(hosts_list)
    args_list = []
    for i in range(n):
        args_list.append((hash_files, hostname, hosts_list, i))

    with Pool(20) as p:
        pool_res = p.starmap(write_shufffiles, args_list)
...
def deploy_file(file, host, target_path, protocol="scp", source_host=None):
    ...
    if host == source_host:
        ...
        if protocol == "scp-r":
            ...
            cmd = ["sftp", "-b", BASE_PATH + "/sftp_cmd2.txt", f"{host}:{target_path}"]
        else:
...
def shuff_thread(shuffle_path, curr_host, dirname, shuff_rcv_path):
    ...
    (_, _, filenames) = next(os.walk(shuffle_path + dirname))
    stdout = []
    stderr = []
    for filename in filenames:
        filename = dirname + "/" + filename
        out, err = deploy_file(
            filename, dirname, shuff_rcv_path, protocol="scp", source_host=curr_host
        )
        stdout += [out]
        stderr += [err]
    return stdout, stderr
...
def do_shuffle(curr_host, hostfile, shuffle_path):
    """For each file in shuffle_path, send the file to host according to hashcode"""
    ...
    for file in filenames:
        with open(os.path.join(shuff_path, file)) as f:
            ...
            for line in file_content:
                if len(line.split()) < 2:
                    erprint(f"Error in file {file} on {curr_host}")
